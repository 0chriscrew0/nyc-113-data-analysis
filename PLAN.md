...
> **Internal working title vs. public title.** Use the question above as the repo/dashboard/portfolio headline — it's what makes someone click. "Response time equity analysis" is the description, not the hook.
...
### In scope
- **Dataset:** NYC 311 Service Requests from 2010 to Present (NYC Open Data, Socrata ID `erm2-nwe9`). Free, no login, API + bulk CSV.
...
- **Complaint types:** Restricted to 5–8 high-volume, agency-consistent types so comparisons are apples-to-apples. Candidates:
  ...
  - Rodent (DOHMH)
  - Water System (DEP)
- **Primary metric:** Resolution time = `closed_date − created_date`, in hours.
...
### Single-dataset note
The 311 file is the only substantive dataset. Income enters as a small ZIP→median-income lookup (~180 rows). If you want to keep this strictly one-source, substitute **borough** or **community board** for income as the grouping variable — the analysis structure is identical, the framing becomes "geographic equity" rather than "income equity," and it loses some punch. Recommendation: keep the ACS join and be transparent about it.
...
*Why this matters:* If H2 holds and H1 fails, that is a **genuinely interesting finding** and a stronger portfolio piece than confirming a naive gap. Do not treat this as a failed project.
...
All of H1–H3 are pairwise. Pairwise relationships are not a conclusion, because income is correlated with borough, complaint mix, and reporting behavior simultaneously. The project's actual finding comes from one model:
...
*Why this is the point:* It lets you write one of these two sentences, and either is a real conclusion —
...
Simple correlations cannot produce either sentence. Do not ship this project without this model.

**Reporting channel — covariate, not a hypothesis.**
Whether `open_data_channel_type` (phone/app/online) varies by income is mildly interesting but not central, and a full standalone analysis of it is scope creep. Include it as a covariate in H4's model and report whether it mattered. One paragraph, not a section.

**Optional stretch: survival analysis.**
H3 (non-closure) and H1 (resolution time) are really the same problem — never-closed tickets are *right-censored observations*, and dropping them biases the median downward unevenly across ZIPs. A Cox proportional hazards model handles both at once and is the statistically correct treatment. This is above the bar for a data analyst portfolio, so treat it as a bonus: if you do it, it's a genuinely strong differentiator and an excellent interview topic. If time is tight, H3 + H4 as specified is sufficient — but *mention the censoring issue in the limitations section either way*, because noticing it is most of the credit.
...
| **DuckDB** | Query the full multi-GB CSV without loading into memory. Reads CSV/Parquet directly with SQL. This is the right call at 30M+ rows and is worth showing off. |
| **SQL** | All aggregation logic. Write it in SQL rather than pandas groupbys — recruiters screen for SQL and it's more legible in a portfolio. |
| **scipy / statsmodels** | Spearman correlation, logistic regression, confidence intervals |
| **Tableau Public** | The primary deliverable. Choropleth map of NYC ZIPs colored by median response time, with complaint-type filter; scatter of income vs. response time; small-multiples trend by year. Published publicly and embedded/linked on the portfolio site. |
...
**Note on Tableau:** Tableau Public cannot handle 30M rows well. Do all heavy aggregation in DuckDB and export a **pre-aggregated extract** (roughly: ZIP × complaint type × year × metrics — a few thousand rows). This is the correct architecture and worth one paragraph in the README, because "I aggregated upstream so the BI layer stayed fast" is a real engineering judgment call.
...
### Phase 1 — Acquisition
- [ ] Download the 311 dataset. Two options: (a) NYC Open Data export UI, (b) Socrata API with `$where=created_date between '2022-01-01' and '2025-12-31'` and pagination. API is cleaner and reproducible — script it.
- [ ] Verify row count and date coverage against the portal's stated totals.
- [ ] Convert CSV → Parquet immediately (`duckdb` one-liner). Cuts file size ~5x and speeds every subsequent query.
...
- [ ] **Flag and quarantine bad rows:** `closed_date` < `created_date` (data entry errors), resolution times < 1 minute (auto-closures), resolution times > 365 days (stale tickets). Record how many of each. Do not delete silently.
...
- [ ] Volume by complaint type by year — confirm the chosen types are stable over the window.
- [ ] Distribution of `resolution_hours` per complaint type. Expect heavy right skew → **use median, not mean**, and state why.
...
- [ ] **Stratified gap:** same, computed *within* each complaint type. Compare to the raw gap → answers H2.
- [ ] Spearman ρ (income vs. median resolution time) per complaint type, with p-values and bootstrap CIs → answers H1.
- [ ] Logistic regression on non-closure → answers H3.
- [ ] **Build the multivariate model → answers H4.** Fit in three nested steps and report all three, so the reader watches the coefficient move:
...
- [ ] Export aggregated extract from DuckDB (ZIP × complaint_type × year → median hours, count, non-closure rate, median income).
- [ ] Get NYC ZIP boundary shapefile (NYC Open Data, ZIP Code Tabulation Areas) and confirm it joins cleanly to your ZIP field.
...
- [ ] Assemble into a dashboard with a clear title stating the finding (not "NYC 311 Dashboard" — something like "Heat complaints take X% longer to close in the lowest-income quintile").
...
- [ ] **Limitations section** covering at minimum: closure timestamp ≠ actual problem resolution; agencies differ in closing practice; reporting-rate differences mean the data measures *reported* problems, not problems; ZCTA≠ZIP is an approximation; observational, not causal.
- [ ] **Write an "Unexpected Findings" section.** Keep a running note file from Phase 3 onward of anything that surprised you — a complaint type that runs backwards to the trend, a discontinuity at a specific date, a borough that behaves unlike the rest. Three bullets is enough. This is the most quotable part of a portfolio project and it costs nothing if you're taking notes as you go. Don't try to write it retroactively at the end; you'll have forgotten.
...
- [ ] **Prep your own answers** to: Why median over mean? Why Spearman over Pearson? Why log-transform? Why DuckDB over pandas? What would change your conclusion? These will be asked, and the project is only as good as your ability to defend the decisions in it.
...
**Resume bullet (fill in the bracketed parts only after you have the numbers):**
...
1. **Count the rows before you claim a number.** Don't state a record count you haven't computed — four years of 311 data is somewhere in the multi-million range, but the exact figure depends on your date filter and how many rows survive cleaning. An interviewer who knows the dataset will ask, and "I'm not sure, I saw that number somewhere" is a bad moment. Run the count, use the real number.
...
| H1 is not supported | Fine — H2/H3 findings are the story instead. The H4 model produces a publishable conclusion either way. Plan the write-up to accommodate a null result. |
